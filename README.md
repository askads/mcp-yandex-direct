...
MCP-сервер для **API Яндекс Директа v5**: управляйте контекстной рекламой из Claude, Cursor, Codex и других AI-клиентов на естественном языке.
...
<img src="docs/demo.gif" alt="Демо: один вопрос — ассистент вызывает list_campaigns, get_statistics и get_balance и находит кампанию, сливающую бюджет" width="1000">

<sub>Настоящая MCP-сессия: реальный сервер, хендшейк и tools/call по stdio через официальный SDK; ответы Яндекс Директа — записанные фикстуры (<a href="docs/demo">docs/demo</a>), поэтому демо воспроизводится без токена и сети — <a href="docs/demo.tape">vhs docs/demo.tape</a>.</sub>
...
- **Защита от случайных записей** — универсальный `raw_request` блокирует любой метод, кроме чтения, без `confirmWrite=true`; отдельные инструменты записи (ставки, бюджеты, минус-слова, статусы) проходят штатное подтверждение MCP-клиента перед вызовом.
...
- **Устойчивость** — повторы с нарастающей паузой при лимитах запросов и ошибках 5xx плюс режим песочницы (`YANDEX_DIRECT_SANDBOX`) для безопасных тестов.
...
Локальный сервер через npx — полный доступ, включая инструменты записи. Разверните своего клиента:
