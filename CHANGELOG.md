...
- Сервер больше не завершается при отсутствии `YANDEX_DIRECT_TOKEN`: он стартует, отвечает
  ...
  («Требуется переменная окружения YANDEX_DIRECT_TOKEN.») сохранён дословно и возвращается
  теперь при вызове инструмента (`CredentialsError`, ответ с `isError`) — без ретраев и без
  обращения к сети. Раньше процесс умирал до MCP-хендшейка, и пользователь видел только
  «красный крест» без причины.
