...
- Подключение аккаунта прямо из диалога — 4 новых инструмента: `auth_status`, `start_login`,
  `finish_login`, `logout`. `start_login` возвращает ссылку на Яндекс OAuth (PKCE S256,
  публичный клиент — без `client_secret` в пакете), пользователь присылает код подтверждения,
  `finish_login` обменивает его на токен и сохраняет в
  `~/.config/mcp-yandex-direct/credentials.json` с правами 0600. Вход срабатывает сразу,
  без перезапуска клиента: токен резолвится при каждом запросе, а не кэшируется. Доступ
  продлевается автоматически по refresh-токену (при истечении и при ошибке авторизации 53).
  `YANDEX_DIRECT_TOKEN` стал необязательным и имеет приоритет над сохранённым входом;
  `logout` удаляет только сохранённый токен и не трогает переменную окружения. Своё
  OAuth-приложение подставляется через `YANDEX_DIRECT_OAUTH_CLIENT_ID`.

### Изменено

- Сервер больше не завершается при отсутствии `YANDEX_DIRECT_TOKEN`: он стартует, отвечает
  на `initialize` и `tools/list`, а в `instructions` предупреждает модель и называет оба пути
  подключения — вход из диалога через `start_login` или переменная окружения с перезапуском.
  Вызов инструмента данных без токена возвращает `AuthRequiredError` (ответ с `isError`)
  с теми же двумя путями — без ретраев и без обращения к сети. Раньше процесс умирал до
  MCP-хендшейка, и пользователь видел только «красный крест» без причины.
- Телеметрия: у выжившего неконфигурированного старта своё событие `unconfigured_start`
  ...
  установки. `startup_failed` остаётся для невалидных значений конфигурации и отправляется
  fire-and-forget — процесс больше не ждёт пинг перед выходом.
