...
/**
 * Ask Ads' OAuth app for Yandex Direct («Яндекс Директ — A1-x-Tech MCP»),
 * scoped to `direct:api` only. Overridable so a user who wants their own app
 * (own consent screen, own rights) can point the flow at it without forking.
 */
export const DEFAULT_CLIENT_ID = "aaf093af4a92428f9192cb824b4e3baf";
 ...
 * the Direct API: without `scope` Yandex issues a token carrying **every** right
 * the app holds, so an app that gains a right later would silently widen every
 * token minted afterwards.
 */
export const SCOPE = "direct:api";
...
export function oauthClientId(): string {
  return process.env.YANDEX_DIRECT_OAUTH_CLIENT_ID || DEFAULT_CLIENT_ID;
}
...
/**
 * No `state` parameter: it exists to correlate a redirect back to the app, and
 * this flow has no redirect — Yandex shows the code on its own page and the
 * user carries it over by hand. The PKCE verifier is what ties the exchange to
 * this process.
 */
  ...
  switch (code) {
    ...
    case "invalid_client":
      return (
        ...
        "Если задан YANDEX_DIRECT_OAUTH_CLIENT_ID — проверьте его; " +
